...
class AxentaApi:
        ...
        for attempt in range(retries + 1):
            try:
                if method == 'GET':
                    ...
                    response = requests.post(self.api_url + uri, json=data, headers={'Authorization': f'Token {token}'})
                else:
                    return None
                result = response.json()
    ...
    def get_address_decorator(self, x: float, y: float):
        data = {
            'coordinates': [
                {
                    'lat': x,
        ...
        result = self.make_request('POST', f'geocoding/reverse/', data)
        return result